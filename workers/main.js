...
pipe.on('data', async (data) => {
  ...
  try {
    ...
    } else if (msg.cmd === 'update-score') {
      if (session) await session.updateScore(msg.matchId, msg.a, msg.b)
    } else if (msg.cmd === 'finish-match') {
      if (session) await session.finishMatch(msg.matchId)
